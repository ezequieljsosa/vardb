...
class DbIO(object):
    ...
    def create_db(self):
        ...
        for t in DbIO.tables:
            t.create_table()
        Allele._schema.create_foreign_key(Allele.main_effect)
    ...
    def add_variant(self, ref_organism, vc, var, effects):
        variant_query = Variant.select().where(
            (Variant.pos == var.POS) & (Variant.contig == var.CHROM) & (Variant.ref_organism == ref_organism) & (
                    Variant.ref == var.REF))
        ...
        for alt in var.ALT:
            ...
            if not allele_query:
                ...
                for effect in effects:
                    ...
                    new_effect = Effect(allele=new_allele, transcript=effect.gene,
    ...
    def load_variants(self, variants, ref_organism, sample, total):
        '''
        ...
        for var, effects in tqdm(variants, total=total):
            with sqldb.atomic():
    ...
    def load_vcf(self, vcf_file, ref_organism, sample=None):
        ...
        self.load_variants(VcfSnpeffIO.parse(vcf_file), ref_organism, sample, total)
